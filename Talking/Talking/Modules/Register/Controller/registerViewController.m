...
- (void)viewDidLoad {
    ...
    [returnButton setTitle:@"已有账号?立即登录" forState:UIControlStateNormal];
...
- (void)Register {
    NSString *codeString = _codeNumber.text;
    NSString *passwordString = [_passWordNubmer.text ea_stringByMD5Bit32];
    NSString *phoneNumberString = _phoneNumber.text;
    NSDictionary *dic = @{@"country_code":@"86",
                          @"phone":phoneNumberString,
                          @"code":codeString,
                          @"password":passwordString};
    NSURL *url = [NSURL URLWithString:@"http://app.ry.api.renyan.cn/rest/phone_register/submit"];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:@"application/json;charset=utf-8" forHTTPHeaderField:@"Content-Type"];
    [request setValue:@"103" forHTTPHeaderField:@"Content-Length"];
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(),^{
            if (!error) {
                ...
                if ([[result objectForKey:@"msg"] isEqualToString:@"验证成功，注册成功"]) {
...
- (void)getCode {
    ...
    NSDictionary *dic = @{@"country_code":contryString,
                          ...
                          @"channelId":@"5",
                          @"osType":@"2"};
    NSURL *url = [NSURL URLWithString:@"http://app.ry.api.renyan.cn/rest/phone_register/new"];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:@"application/json;charset=utf-8" forHTTPHeaderField:@"Content-Type"];
    [request setValue:@"66" forHTTPHeaderField:@"Content-Length"];
    [request setValue:@"okhttp/2.5.0" forHTTPHeaderField:@"User-Agent"];
    request.HTTPMethod = @"POST";
    request.timeoutInterval = 60.f;
    NSData *data = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:nil];
    NSString *jsonString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
