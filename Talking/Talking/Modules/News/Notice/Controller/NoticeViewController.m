...
- (void)GetMessage {
    ...
    NSString *url = [NSString stringWithFormat:@"http://app.ry.api.renyan.cn/rest/auth/message/query_all_reply_sysmsg?to_uid=%@&limit=10",_uid];
...
- (void)addMoreMessage {
    ...
    [manager.requestSerializer setValue:_token forHTTPHeaderField:@"X-AuthToken"];
    [manager.requestSerializer setValue:@"j8slb29fbalc83pna2af2c2954hcw65" forHTTPHeaderField:@"X-ApiKey"];
    NSString *url = [NSString stringWithFormat:@"http://app.ry.api.renyan.cn/rest/auth/message/query_all_reply_sysmsg?to_uid=%@&last_mid=%@&limit=10",_uid,[_midArray lastObject]];
    [manager GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSArray *array = [responseObject objectForKey:@"messages"];
        for (NSDictionary *dic in array) {
            NSDictionary *subDic = [dic objectForKey:@"message"];
